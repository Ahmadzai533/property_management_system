...
import { 
  Building2, Plus, Eye, Edit, Home, List, LayoutGrid, 
  Grid, Package, Users, DollarSign, Calendar, AlertCircle 
} from 'lucide-react';
...
const AllUnits = () => {
  ...
  const handleAddUnit = useCallback(() => {
    navigate('/properties/units/add');
  }, [navigate]);

  const handleViewUnit = useCallback((id) => {
    navigate(`/properties/units/${id}`);
  }, [navigate]);

  const handleEditUnit = useCallback((id) => {
  ...
  const handleArchiveUnit = useCallback((id) => {
  ...
  const breadcrumbItems = useMemo(() => [
    ...
    { label: 'All Units', path: '/properties/units', active: true },
  ], []);

  const firstUnitId = units.length > 0 ? units[0].id : null;

  // Custom stats for units
  const unitStats = {
    total: stats.total || 0,
    ...
    totalRentValue: stats.totalRentValue || 0,
    averageRent: stats.averageRent || 0,
    occupancyRate: stats.occupancyRate || 0,
    maintenanceRate: stats.maintenanceRate || 0,
  };
      ...
      <div className="px-4 sm:px-6 lg:px-8 pt-2 pb-4">
        <div className="rounded-2xl bg-gradient-to-r from-[#6D28D9] to-[#8B5CF6] p-5 text-white shadow-lg dark:from-[#6D28D9] dark:to-[#8B5CF6]">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white/80 mt-0.5">Manage and monitor all units across your property portfolio</p>
              </div>
            </div>
          ...
          <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-white/20">
            ...
            <button
              onClick={() => navigate('/properties/units/add')}
              className="flex items-center gap-2 px-3 py-1.5 bg-white/10 hover:bg-white/20 transition-colors rounded-lg text-white text-sm"
            >
              <Plus className="h-4 w-4" />
              Add New
            ...
            {firstUnitId && (
              <>
                <button
                  onClick={() => navigate(`/properties/units/${firstUnitId}`)}
                  className="flex items-center gap-2 px-3 py-1.5 bg-white/10 hover:bg-white/20 transition-colors rounded-lg text-white text-sm"
                >
                ...
                <button
                  onClick={() => navigate(`/properties/units/edit/${firstUnitId}`)}
                  className="flex items-center gap-2 px-3 py-1.5 bg-white/10 hover:bg-white/20 transition-colors rounded-lg text-white text-sm"
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
                    {units.map((unit) => (
                      <div key={unit.id} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 hover:shadow-lg transition-shadow">
                        <img src={unit.image} alt={unit.unitNumber} className="w-full h-48 object-cover rounded-lg mb-3" />
                        <div className="flex items-center justify-between">
                          <h3 className="font-semibold text-gray-900 dark:text-white">Unit {unit.unitNumber}</h3>
                          <span className={`px-2 py-1 rounded-full text-xs ${
                            unit.status === 'Occupied' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' :
                            unit.status === 'Vacant' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' :
                            unit.status === 'Maintenance' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300' :
